...
#define cur_node (reinterpret_cast<const ast::Program*>(node))
            ...
            printf("type = Program\n");
            put_tab(dep + 1);
            printf("name = %s\n", convertToString(cur_node->head->id->ident).c_str());
            // todo: remove const cast
            put_tab(dep + 1);
            printf("param(ids) = \n");
            printAST(cur_node->head->id_list, dep + 1);
            put_tab(dep + 1);
            printf("const decls = \n");
            printAST(cur_node->body->const_decls, dep + 1);
            put_tab(dep + 1);
            printf("var decls = \n");
            printAST(cur_node->body->var_decls, dep + 1);
            put_tab(dep + 1);
            printf("subprogram decls = \n");
            printAST(cur_node->body->prog_decls, dep + 1);
            put_tab(dep + 1);
            printf("body = \n");
            printAST(cur_node->body->compound, dep + 1);
...
#define cur_node (reinterpret_cast<const ast::Subprogram*>(node))
            ...
            put_tab(dep + 1);
            printf("name = %s\n", convertToString(cur_node->head->name).c_str());
            // todo: remove const cast
            put_tab(dep + 1);
            printf("param(ids) = \n");
            if (cur_node->head) printAST(cur_node->head->decls, dep + 1);
            put_tab(dep + 1);
            if (cur_node->body) {
                printf("const decls = \n");
                printAST(cur_node->body->const_decls, dep + 1);
...
#define cur_node (reinterpret_cast<const ast::ExpCall*>(node))
            ...
            if (cur_node->params) {
                for (auto exp: cur_node->params->vec) {
                    printAST(exp, dep + 1);
                }
            }
...
//            printf("invoke_target = %s\n", convertToString(cur_node->fn).c_str());
            ...
            if (cur_node->exp_list)
                for (auto exp: cur_node->exp_list->vec) {
                    printAST(exp, dep + 1);
                }
...
#define cur_node (reinterpret_cast<const ast::ForStatement*>(node))
            ...
            put_tab(dep + 1);
            printf("loop_var = %s\n", convertToString(cur_node->loop_var).c_str());
            put_tab(dep + 1);
            printf("from_exp = \n");
            printAST(cur_node->from_exp, dep + 1);
            put_tab(dep + 1);
            printf("to_exp = \n");
            printAST(cur_node->to_exp, dep + 1);
            put_tab(dep + 1);
            printf("body = \n");
...
#define cur_node (reinterpret_cast<const ast::Variable*>(node))
            ...
            printf("exps =\n");
            if (cur_node->id_var) {
                for (auto param: cur_node->id_var->vec) {
                    printAST(param, dep + 1);
                }
            }
